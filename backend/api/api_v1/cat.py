...
@router.post('/', response_model=CatDB, status_code=status.HTTP_201_CREATED)
...
async def get_cats(
    session: AsyncSession = Depends(get_async_session),
    search: str | None = Query(default=None, description=CAT_SEARCH_DESCR),
    breed_id: int | None = Query(
        ge=0, default=None, description=CAT_BREED_SEARCH_DESCR
    ),
    sex: list[CatSex] = Query(default=[], description=CAT_SEX_DESCR),
    color: list[CatColor] = Query(default=[], description=CAT_COLOR_DESCR),
    limit: int = Query(
...
@router.patch(
    '/{id}/', response_model=CatDB, status_code=status.HTTP_201_CREATED
)
